...
const AIChatWidget = () => {
    ...
    const [messages, setMessages] = useState([
        { role: 'ai', text: 'Chào quý khách, tôi là trợ lý ảo của Gốm Sứ Đại Thành. Quý khách cần tìm hiểu về loại gốm sứ nào ạ?' }
    ]);
    ...
    const fetchHistory = async (id) => {
        try {
            const response = await aiApi.getHistory(id);
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'ai', text: 'Xin lỗi, hiện tại tôi đang bận pha trà. Quý khách vui lòng thử lại sau giây lát!' }]);
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <div className="w-96 bg-white border border-gold/20 shadow-premium flex flex-col animate-in slide-in-from-bottom-10 duration-300">
                        ...
                        {messages.map((msg, idx) => (
                            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-[85%] p-3 rounded-none border ${msg.role === 'user'
                                    ...
                                    <p className="text-xs leading-relaxed font-body whitespace-pre-wrap">{msg.text}</p>
                                </div>
                            </div>
                        ))}
                        {isLoading && (
                    ...
                    <form onSubmit={handleSend} className="p-4 border-t border-gold/10 bg-white shadow-inner flex items-center gap-2">
                        <input
                            ...
                            placeholder="Hỏi về bình hoa, gốm tâm linh..."
