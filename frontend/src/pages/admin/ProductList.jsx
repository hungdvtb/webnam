...
const ProductList = () => {
    ...
    return (
        <div className="absolute inset-0 flex flex-col bg-[#fcfcfa] animate-fade-in p-6 z-10 w-full h-full">
            ...
            {notification && (
                <div className={`fixed top-6 right-6 z-[100] p-4 rounded-md shadow-2xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4 duration-300 ${notification.type === 'error' ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200'}`}>
            ...
            <div className="flex-none bg-[#F8FAFC] pb-4 space-y-2">
                ...
                <div className="bg-white border border-primary/10 p-2 shadow-sm rounded-sm flex flex-wrap items-center gap-2">
                    <div className="flex gap-1 items-center">
                        {!isTrashView && (
                            <button onClick={() => navigate('/admin/products/new')} className="bg-brick text-white p-1.5 hover:bg-umber transition-all rounded-sm w-9 h-9" title="Thêm sản phẩm mới"><span className="material-symbols-outlined text-[18px]">add</span></button>
                        )}
                ...
                <div ref={filterRef} className="bg-white border border-primary/20 p-5 shadow-2xl mb-4 rounded-sm animate-in slide-in-from-top-4 duration-300 relative z-50 text-[#0F172A]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-6">
                        <div className="space-y-1.5">
                            <label className="text-[13px] font-medium text-[#0F172A]">Danh mục</label>
                            <select name="category_id" value={tempFilters.category_id} onChange={handleTempFilterChange} className="w-full h-10 bg-primary/5 border border-primary/20 rounded-sm px-3 text-[13px] font-bold text-[#0F172A] focus:outline-none focus:border-primary transition-all appearance-none cursor-pointer">
                                <option value="">Tất cả danh mục</option>
                                {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                            </select>
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[13px] font-medium text-[#0F172A]">Loại sản phẩm</label>
                            <select name="type" value={tempFilters.type} onChange={handleTempFilterChange} className="w-full h-10 bg-primary/5 border border-primary/20 rounded-sm px-3 text-[13px] font-bold text-[#0F172A] focus:outline-none focus:border-primary transition-all appearance-none cursor-pointer">
                                <option value="">Tất cả loại hình</option>
                                {Object.entries(TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}
                            </select>
                        </div>
                        <div className="space-y-1.5 md:col-span-1 lg:col-span-1">
                            <label className="text-[13px] font-medium text-[#0F172A]">Tồn kho</label>
                            <div className="flex items-center gap-2 h-10">
                                <input type="number" name="min_stock" placeholder="Từ" className="w-1/2 h-full bg-primary/5 border border-primary/20 rounded-sm px-3 text-[13px] font-bold text-[#0F172A] focus:outline-none focus:border-primary" value={tempFilters.min_stock} onChange={handleTempFilterChange} />
                                <span className="text-primary/30 font-bold">-</span>
                                <input type="number" name="max_stock" placeholder="Đến" className="w-1/2 h-full bg-primary/5 border border-primary/20 rounded-sm px-3 text-[13px] font-bold text-[#0F172A] focus:outline-none focus:border-primary" value={tempFilters.max_stock} onChange={handleTempFilterChange} />
                            </div>
                        </div>
                        <div className="space-y-1.5 pb-1">
                            <label className="text-[13px] font-medium text-[#0F172A]">Ngày tạo (Từ)</label>
                            <input type="date" name="start_date" className="w-full h-10 bg-primary/5 border border-primary/20 rounded-sm px-3 text-[13px] font-bold text-[#0F172A] focus:outline-none focus:border-primary cursor-pointer" value={tempFilters.start_date} onChange={handleTempFilterChange} />
                        </div>
                        <div className="space-y-1.5">
                            <label className="text-[13px] font-medium text-[#0F172A]">Ngày tạo (Đến)</label>
                            <input type="date" name="end_date" className="w-full h-10 bg-primary/5 border border-primary/20 rounded-sm px-3 text-[13px] font-bold text-[#0F172A] focus:outline-none focus:border-primary cursor-pointer" value={tempFilters.end_date} onChange={handleTempFilterChange} />
                        </div>
                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-8">
                                {allAttributes.filter(a => a.is_filterable).map(attr => (
                                    <div key={attr.id} className="space-y-3">
                                        <label className="text-[13px] font-medium text-[#111]">{attr.name}</label>
                                        <div className="flex flex-wrap gap-1.5">
                                            {attr.options?.map(opt => (
                                                <button 
                                                    key={opt.id} 
                                                    onClick={() => handleTempAttributeFilterChange(attr.id, opt.value)} 
                                                    className={`px-2.5 py-1 text-[12px] font-bold rounded-sm border transition-all ${tempFilters.attributes[attr.id]?.includes(opt.value) ? 'bg-primary text-white border-primary shadow-sm' : 'bg-white text-primary/60 border-primary/20 hover:border-primary/40 hover:text-primary'}`}
                                                >
                                                    {opt.value}
                                                </button>
                                            ))}
